...
import { lastFocusableElement } from '@/lib/lastFocusableElement'
...
export class MegaMenu extends BaseElementWithoutShadowDOM {
  ...
  constructor() {
    ...
    this._handleMenuFocusOut = this._handleMenuFocusOut.bind(this)
  ...
  connectedCallback(): void {
    ...
    Array.from(this.menus).forEach((menu) => {
      ...
      lastFocusableElement(menu)?.addEventListener(
        'keydown',
  ...
  _handleMenuFocusOut(event: KeyboardEvent): void {
  ...
  protected updated(
    ...
    if (_changedProperties.has('activeElement')) {
      ...
      this._handleMenuDrawerARIATags()
    }
  }
  ...
  _handleMenuDrawerARIATags() {
    for (const menu of Array.from(this.menus)) {
      if (!this.activeElement) {
        menu.setAttribute('aria-hidden', 'true')
        continue
      }
      // Toggle Aria Hidden state based on if this.activeElement menu trigger is set or not
      const targetMenuId = this.activeElement.getAttribute('aria-controls')
