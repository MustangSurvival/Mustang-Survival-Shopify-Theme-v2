...
export class DeferredMedia extends BaseElementWithoutShadowDOM {
  ...
  poster!: Promise<HTMLElement | null>
  ...
  get isInlineHTML5LoopingVideo(): boolean {
    return this.mediaType === 'video' && this.loop && this.autoplay
  }

  protected firstUpdated(_changedProperties: PropertyValues): void {
    super.firstUpdated(_changedProperties)

    if (this.isInlineHTML5LoopingVideo) {
  ...
  protected updated(
    ...
    if (this.isInlineHTML5LoopingVideo) return
  ...
  pause(skipEvent = false): void {
    ...
    this.playing = false
    !this.isInlineHTML5LoopingVideo && skipEvent && this.#_pause()
  }
