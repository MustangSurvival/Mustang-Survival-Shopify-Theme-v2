...
export class PredictiveSearch extends BaseElementWithoutShadowDOM {
  ...
  updateSearchForTerm(previousTerm, newTerm) {
    const currentButtonText = this.searchForTextElement?.innerText
    if (currentButtonText) {
      ...
      this.searchForTextElement.innerText = newButtonText
    }
  }
  ...
  switchOption(direction) {
    ...
    this.selectedElement = this.querySelector('[aria-selected="true"]')

    // Filter out hidden elements (duplicated page and article resources) thanks
    // to this https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/offsetParent
    const allVisibleElements = Array.from(
      this.querySelectorAll('li, [data-predictive-search-item]')
  ...
  getSearchResults(searchTerm) {
    ...
    fetch(
      `${window.routes.predictive_search_url}?q=${encodeURIComponent(searchTerm)}&section_id=predictive-search`,
      {
  ...
  getResultsMaxHeight() {
    this.resultsMaxHeight =
      window.innerHeight -
      document.querySelector('#header-search').getBoundingClientRect().bottom
