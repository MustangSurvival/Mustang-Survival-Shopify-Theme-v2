  ...
  abstract class WithShopifyCartClientClass extends superClass {
    ...
    async addItemToCart(items: CartItemPayload[]) {
      try {
        ...
        await this._handleCartAdd(
          [deltaItems.items] as unknown as ShopifyCartLineItem[],
          this
        )
    ...
    async addItemToWithForm(formData: FormData) {
      try {
        ...
        await this._handleCartAdd(
          [deltaItems] as unknown as ShopifyCartLineItem[],
          this
        )
        await this._handleCartChange()
        deltaItems.sections &&
    ...
    async _handleCartAdd(items?: ShopifyCartLineItem[], target?: BaseElement) {
      this.$emit('cart:items-added', {
        items,
        target,
      } satisfies AddToCartEventPayload)
    }
